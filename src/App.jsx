...
function BulkImport() {
  ...
  const PO_COLS=[{key:"order_no",label:"Order No",req:true},{key:"material_no",label:"Material No",req:true},{key:"description",label:"Description"},{key:"qty",label:"Qty",req:true,num:true},{key:"uom",label:"UoM"},{key:"start_date",label:"Start Date"},{key:"end_date",label:"End Date"},{key:"status",label:"Status"}];
  const RM_COLS=[{key:"material_no",label:"Material No",req:true},{key:"op_no",label:"Op No",req:true,num:true},{key:"work_center",label:"Work Center",req:true},{key:"op_description",label:"Op Description"},{key:"setup_time",label:"Setup Time (min)",num:true},{key:"machine_time",label:"Machine Time (min)",req:true,num:true},{key:"labor_time",label:"Labor Time (min)",num:true}];
  ...
  function parseFile(file,cols,setter){ const r=new FileReader(); r.onload=e=>{ try{ const wb=window.XLSX.read(e.target.result,{type:"binary",cellDates:true}); const ws=wb.Sheets[wb.SheetNames[0]]; const raw=window.XLSX.utils.sheet_to_json(ws,{defval:"",raw:false,dateNF:"YYYY-MM-DD"}); const norm=s=>s.toString().toLowerCase().replace(/[^a-z0-9]/g,""); const rows=raw.map((row,i)=>{ const nr={}; Object.keys(row).forEach(k=>{nr[norm(k)]=row[k];}); const data={}; const errors=[]; cols.forEach(col=>{let v=nr[norm(col.label)]??nr[norm(col.key)]??""; data[col.key]=v; if(col.req&&(v===""||v===null))errors.push(col.label+" missing"); if(col.num&&v!==""&&isNaN(Number(v)))errors.push(col.label+" must be a number");}); return{rowNum:i+2,data,errors,valid:errors.length===0}; }); setter(rows); }catch(err){alert("Cannot read file: "+err.message);} }; r.readAsBinaryString(file); }
